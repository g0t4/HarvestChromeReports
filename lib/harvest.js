define(['amplify', 'jquery', 'jquery.base64', 'linq'], function (amplify, $) {
    ...
    module.HarvestClient = function (user, password, harvestUrl) {
        ...
        amplify.request.define("clients", "ajax", client.harvestParameters(clientParameters));
        client.getClientsById = function (success, error) {
            amplify.request({
                resourceId: 'clients',
                success: function (result) {
                    var clients = {};
                    Enumerable.From(result)
                        .Select("$.client")
                        .ForEach(function (c) { clients[c.id] = c; });
                    success(clients);
                },
                error: error
            })
        ...
        client.clearCaches = function () {
            ...
            client.clearCachedResource("clients");
        }
        return client;
    }
    return module;
